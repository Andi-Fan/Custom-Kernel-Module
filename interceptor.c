...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	// checking if the syscall is valid, and not MY_CUSTOM_SYSCALL itself
	if (syscall < 0 || syscall == MY_CUSTOM_SYSCALL || syscall > NR_syscalls - 1){
		return -EINVAL;
	}
	// checking if pid is valid, or 0
	if (pid < 0 || (pid != 0 && pid_task(find_vpid(pid), PIDTYPE_PID) == NULL)) {
		return -EINVAL;
	}

	// Error checks specific to intercepting
	if (cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE){
		...
		if (cmd == REQUEST_SYSCALL_INTERCEPT){
			...
			if (table[syscall].intercepted == 1){
				// cannot intercept already intercepted syscall
				spin_unlock(&my_table_lock);
				return -EBUSY;
			}
			...
			if (table[syscall].intercepted == 0){
				// cannot release a syscall that hasn't been intercepted
				spin_unlock(&my_table_lock);
				return -EINVAL;
			}
	...
	// Error checks specific to monitoring
	if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING) {

		if (current_uid() != 0) {
			if (pid == 0) {
				// must be root to start/stop monitoring ALL pids
				return -EPERM;
			}
			if (check_pids_same_owner(pid, current->pid) != 0){
				// pids must have same owner to start/stop monitoring
				return -EPERM;
			}
		}
		spin_lock(&my_table_lock);
		if (table[syscall].intercepted == 0){
			// cannot start/stop monitoring a syscall that hasnt been intercepted
			spin_unlock(&my_table_lock);
			return -EINVAL;
		}
		spin_unlock(&my_table_lock);
	...
	if (cmd == REQUEST_START_MONITORING) {
		...
		if (check_pid_monitored(syscall, pid) == 0 && pid != 0) {
			// cannot stop monitoring a pid that is not being monitored
			// unless it is 0
			spin_unlock(&my_table_lock);
			return -EINVAL;
		}
